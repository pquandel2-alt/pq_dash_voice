...
const outputs = [
  path.join(root, 'app/src/main/assets/avatar-target.json'),
  path.join(root, 'app/src/main/assets/avatar-geometry.json'),
];
...
const faceAmber = intensity => {
  const hot = smoothstep((intensity - 0.94) / 0.06);
  return rgb(255, 88 + intensity * 122 + hot * 45, 5 + intensity * 42 + hot * 108);
};
...
// One explicit front silhouette shared by every head region. v=0 is crown, v=1 chin.
// Unlike the previous mask profile, neither endpoint collapses toward zero: the crown stays
// broad and rounded while cheek -> jaw -> chin narrows like a compact humanoid face.
const HEAD_BOUNDARY = [
  [0.00, 0.64], // crown edge; rounded cap raises the centre above it
  [0.10, 0.82], // upper skull
  [0.20, 0.94],
  [0.34, 0.99], // temple
  [0.46, 1.00], // cheek
  [0.58, 0.97],
  [0.70, 0.89], // lower cheek
  [0.80, 0.77], // jaw transition
  [0.90, 0.64], // jaw
  [1.00, 0.54], // soft, broad chin
];

function profileAt(v) {
  const t = clamp(v, 0, 1);
  for (let i = 0; i < HEAD_BOUNDARY.length - 1; i++) {
    const a = HEAD_BOUNDARY[i], b = HEAD_BOUNDARY[i + 1];
    if (t >= a[0] && t <= b[0]) {
      const local = smoothstep((t - a[0]) / (b[0] - a[0]));
      return lerp(a[1], b[1], local);
    }
  }
  return HEAD_BOUNDARY.at(-1)[1];
}

// The width curve deliberately stays broad at v=0; this shared crown lift closes that broad
// upper-skull section with a shallow human dome instead of a flat helmet edge.
function headYAt(v, xNorm) {
  const crownInfluence = 1 - smoothstep(v / 0.18);
  const xn = clamp(xNorm, -1, 1);
  const dome = Math.sqrt(Math.max(0, 1 - xn * xn));
  const chinInfluence = smoothstep((v - 0.84) / 0.16);
  return HEAD_TOP + v * HEAD_HEIGHT - crownInfluence * dome * CROWN_DOME +
    chinInfluence * dome * CHIN_DOME;
}

const CX = 0.5;
const HEAD_TOP = 0.190;
const HEAD_HEIGHT = 0.250;
const CROWN_DOME = 0.038;
const CHIN_DOME = 0.018;
const HEAD_BOTTOM = HEAD_TOP + HEAD_HEIGHT;
const VIEW_ASPECT = 1.6;
const HEAD_MAX_WIDTH = 0.148;
...
const CORE_CY = 0.302;
const CORE_WIDTH = 0.096;
const CORE_HEIGHT = 0.170;
...
function shoulderControls(layer) {
  return {
    p0x: 0.008 + layer * 0.036,
    p0y: CHEST_CORE_Y + 0.010 + layer * 0.100,
    p1x: 0.024 + layer * 0.026,
    p1y: 0.468 + layer * 0.115,
    p2x: 0.098 + layer * 0.027,
    p2y: 0.535 + layer * 0.125,
    p3x: SHOULDER_HALF - layer * 0.026,
    p3y: SHOULDER_OUTER_Y + layer * 0.105,
  };
}
...
for (const side of [-1, 1]) {
  for (let layer = 0; layer < 2; layer++) {
    for (let point = 0; point < 70; point++) {
      ...
      const y = u * u * u * (HEAD_BOTTOM + 0.003) + 3 * u * u * t * 0.475 +
        3 * u * t * t * 0.560 + t * t * t * SHOULDER_OUTER_Y + layer * 0.0025;
...
for (const side of [-1, 1]) {
  for (let band = 0; band < 13; band++) {
    ...
    for (let point = 0; point < 25; point++) {
      ...
      const p1x = 0.042 + layer * 0.016, p1y = 0.605 + layer * 0.045;
      const p2x = 0.103 + layer * 0.020, p2y = 0.630 + layer * 0.075;
      const p3x = 0.151 - layer * 0.010, p3y = 0.700 + layer * 0.120;
...
for (let strand = -7; strand <= 7; strand++) {
  ...
  for (let point = 0; point < 24; point++) {
    ...
    const y = lerp(0.590, BODY_FADE_Y, t) + Math.sin(strand * 1.31 + t * Math.PI * 1.7) * 0.0028;
...
for (const side of [-1, 1]) {
  for (let trail = 0; trail < 8; trail++) {
    for (let point = 0; point < 46; point++) {
      if ((point + trail * 3) % 11 === 0) continue;
      const t = point / 45;
      add({
        x: CX + side * lerp(0.145, 0.31, t),
        y: 0.34 + trail * 0.036 + Math.sin(t * Math.PI * 1.6 + trail * 0.63 + (side > 0 ? 0.37 : 0)) * (0.018 + trail * 0.0015),
...
for (const field of [
  { region: 'headBack', count: 500, z: -0.030, intensity: 0.32, alpha: 0.24, radius: 0.44 },
  { region: 'headMid', count: 650, z: -0.006, intensity: 0.50, alpha: 0.40, radius: 0.48 },
  { region: 'headFront', count: 500, z: 0.020, intensity: 0.68, alpha: 0.56, radius: 0.51 },
]) {
  for (let i = 0; i < field.count; i++) {
    ...
    add({
      x: CX + boundary * xNorm,
      y: headYAt(v, xNorm) + (random() - 0.5) * 0.0022,
...
for (let i = 0; i < 1180; i++) {
  ...
  add({
    ...
    color: faceAmber(intensity), brightness: 0.50 + intensity * 0.50,
    radius: 0.66 + (1 - intensity) * 0.22 + intensity * 0.28,
...
const HEAD_BANDS = 78;
for (let band = 0; band < HEAD_BANDS; band++) {
  const v = 0.010 + band / (HEAD_BANDS - 1) * 0.980;
  const profile = profileAt(v);
  const boundary = HEAD_HALF * profile;
  const count = Math.max(48, Math.round(58 + profile * 34));
  for (let point = 0; point < count; point++) {
    if ((point + band * 7) % 173 === 0) continue;
    const t = point / (count - 1);
    const xNorm = -1 + t * 2;
    const edge = Math.pow(Math.abs(xNorm), 2.4);
    const yWave = Math.cos(xNorm * Math.PI * 0.72) * (0.00075 + (band % 7) * 0.000035) +
      Math.sin(band * 1.31 + point * 0.57) * 0.00010;
    add({
      x: CX + xNorm * boundary,
      y: headYAt(v, xNorm) + yWave,
...
for (let layer = 0; layer < rimLayers.length; layer++) {
  ...
  for (const side of [-1, 1]) {
    for (let point = 0; point < 154; point++) {
      if (layer === 2 && point % 2 === 1) continue;
      if (side > 0 && (point === 0 || point === 153)) continue;
      const v = point / 153;
  ...
  // Crown cap uses the same v=0 boundary and shared dome equation; it is a dense particle
  // filament, not a separate oval or tube.
  const capPoints = layer === 2 ? 58 : 112;
  for (let point = 0; point < capPoints; point++) {
    const xNorm = -1 + point / (capPoints - 1) * 2;
    const cluster = 0.72 + 0.28 * Math.sin(point * 0.41 + layer * 1.7);
    add({
      x: CX + xNorm * HEAD_HALF * profileAt(0) * rim.scale,
      y: headYAt(0, xNorm) + Math.sin(point * 0.37 + layer) * 0.00035,
      z: rim.z,
      color: layer === 2 ? rgb(205, 252, 255) :
        (layer === 1 ? rgb(0, 220, 255) : rgb(0, 168, 255)),
      brightness: rim.brightness * cluster,
      radius: rim.radius * (0.78 + cluster * 0.28),
      region: rim.region, bandId: 710 + layer, flowT: point / (capPoints - 1), drift: 0.24,
    });
  }

  // Matching soft chin cap; the side profile remains wide while the centre rounds down.
  for (let point = 0; point < capPoints; point++) {
    const xNorm = -1 + point / (capPoints - 1) * 2;
    const cluster = 0.70 + 0.30 * Math.sin(point * 0.39 + layer * 1.3);
    add({
      x: CX + xNorm * HEAD_HALF * profileAt(1) * rim.scale,
      y: headYAt(1, xNorm) + Math.sin(point * 0.31 + layer) * 0.0003,
      z: rim.z,
      color: layer === 2 ? rgb(205, 252, 255) :
        (layer === 1 ? rgb(0, 220, 255) : rgb(0, 168, 255)),
      brightness: rim.brightness * cluster,
      radius: rim.radius * (0.76 + cluster * 0.27),
      region: rim.region, bandId: 720 + layer, flowT: point / (capPoints - 1), drift: 0.24,
    });
  }
}
...
const metrics = {
  ...
  headHeight: +(HEAD_HEIGHT + CROWN_DOME + CHIN_DOME).toFixed(3),
  headHeightToWidth: +((HEAD_HEIGHT + CROWN_DOME + CHIN_DOME) / (HEAD_MAX_WIDTH * VIEW_ASPECT)).toFixed(3),
  ...
  stableBustHeightBelowHead: +(CHEST_CORE_Y - HEAD_BOTTOM).toFixed(3),
  bustToHeadWidth: +((CHEST_CORE_Y - HEAD_BOTTOM) / (HEAD_MAX_WIDTH * VIEW_ASPECT)).toFixed(3),
  faceCoreWidth: CORE_WIDTH,
  faceCoreHeight: CORE_HEIGHT,
  faceCoreToHeadWidth: +(CORE_WIDTH / HEAD_MAX_WIDTH).toFixed(3),
  headBands: HEAD_BANDS,
  headBandSamplesMin: 48,
  headBandSamplesMax: 92,
  shoulderBandsPerSide: 40,
  sideTrailsPerSide: 8,
};
...
for (const output of outputs) fs.writeFileSync(output, geometry);
console.log(`${particles.length} deterministic 2.5D particles -> ${outputs.join(', ')}`);
